...
function renderSetup(playerIndex) {
    ...
    form.addEventListener("submit", (event) => {
        ...
        game.names[playerIndex] = nickname.slice(0, MAX_NICKNAME_LENGTH);
        game.secrets[playerIndex] = value;

        if (playerIndex === 0) {
            renderHandoff(
                `${playerName(0)}'s number is locked`,
                `Pass the device to ${playerName(1)}. ${playerName(0)} should look away while ${playerName(1)} chooses a secret number.`,
                () => renderSetup(1)
            );
        } else {
            renderHandoff(
                "Both numbers are locked",
                `Pass the device to ${playerName(0)}. ${playerName(0)} gets the first turn.`,
                () => {
...
function renderWinner(winner, winningGuess) {
    ...
    const revealBox = view.querySelector(".reveal-box");
    revealBox.innerHTML = "";
    const revealTitle = document.createElement("strong");
    revealTitle.textContent = "Final numbers";
    revealBox.appendChild(revealTitle);
    revealBox.appendChild(document.createElement("br"));
    revealBox.append(
        `${playerName(0)} chose ${game.secrets[0]}`,
        document.createElement("br"),
        `${playerName(1)} chose ${game.secrets[1]}`
    );
